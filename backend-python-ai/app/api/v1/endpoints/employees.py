...
async def get_employees(
    ...
    try:
        ...
        # 获取总数（简化处理，实际应该从服务层获取总数）
        total_employees = len([
            emp_id for emp_id, emp_data in employee_service._employees.items()
            if (not status_filter or emp_data.get("status") == status_filter) and
               (not category or category in emp_data.get("category", [])) and
               (not user_id or emp_data.get("created_by") == user_id)
        ])
...
async def publish_employee(
    ...
    try:
        ...
        # 发布员工
        published_employee = employee_service.publish_employee(employee_id)
